pipeline {
    ...
    stages {
        ...
        stage("Docker Image Build") {
            steps {
                sh "docker build --no-cache -t huahua10ve/apache_boardnext:${BUILD_NUMBER} ./docker/apache2/"
                sh "docker build --no-cache -t huahua10ve/boardrest_boardnext:${BUILD_NUMBER} ./docker/boardrest/"
            }
        }
        stage("Docker Image Push") {
            steps {
                sh "docker push huahua10ve/apache_boardnext:${BUILD_NUMBER}"
                sh "docker push huahua10ve/boardrest_boardnext:${BUILD_NUMBER}"
            }
        }
        stage("Docker Image Clean up") {
            steps {
                sh "docker image rm huahua10ve/apache_boardnext:${BUILD_NUMBER}"
                sh "docker image rm huahua10ve/boardrest_boardnext:${BUILD_NUMBER}"
            }
        }
        stage("Deploy") {
            steps {
